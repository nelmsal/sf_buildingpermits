...
for (start in c(1,len_half)){
  ...
  focus.cols = vars.ivs.cont_fin[start:end]
  focus.cols = focus.vars[sapply(focus.vars, function(item) item %in% colnames(sf.predict))]
  
  sf.cont = 
    sf.predict %>%
    select(c(var.dvs.plot, focus.cols)) %>% 
    st_drop_geometry(.)
