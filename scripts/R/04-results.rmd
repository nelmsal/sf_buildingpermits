...
sf.test =
  ...
  mutate(
    ...
    units1519.ape       = units1519.abserror / ifelse(units.tot.15_19==0, 1, units.tot.15_19),
    units1519.pe         = units1519.error / ifelse(units.tot.15_19==0, 1, units.tot.15_19)
    ) 
...
grid.arrange(ncol=2,
  ggplot(data = sf.test %>% filter(abs(units1519.error)<550)) +
  ...
  ggplot(data = sf.test%>% filter(abs(units1519.abserror)<550)) +
    ...
    labs(subtitle = 
           glue("Absolute Error (MAE = {test.MAE %>% round()})")) +
    plotTheme(),
  ...
  ggplot(data = sf.test %>% filter(units1519.ape<5)) +
    ...
    labs(subtitle = 
           glue("Absolute Percent Error (MAPE = {test.MAPE})")) + 
    plotTheme()
)
...
train.cv.100_final = 
  rbind(
    ...
    data.frame(
      ...
      MAE.mean = sf.predict.cv$units1519.abserror %>% mean() %>% 
        format_nums(),#,
      MAE.sd   = '',
      RMSE.mean = sf.predict.cv$units1519.rmse[1] %>%
        format_nums(),#,
      RMSE.sd   = '',
